...
export function encodeMacroEvent(event) {
    ...
    if (event.action === "move") {
        ...
        if (SPECIAL_MODIFIER_UNICODE.has(u)) { type = 9; code = SPECIAL_HID[u] ?? 0; }
        else {
            type = 10;
...
export function macroBinding(macroIndex, playback) {
    if (playback.mode === 4) return [0x71, macroIndex, playback.count ?? 1];
    return [0x70, macroIndex, playback.mode];
}
/**
 * Rebuild Lingbao's global command-0x15 macro image.
 *
 * IMPORTANT: byte 3 is always 0x12 for the M9 macro format. It is NOT tied
 * to the HID report length. This was confirmed from multiple known-good
 * Lingbao IndexedDB snapshots from an M9 device using 63-byte HID reports.
 *
 * `existing` must contain the complete macro list in device/index order.
 */
export function buildMacroBlob(existing, newEvents) {
    ...
    const header = [0, 0, 0x15, MACRO_COMMAND_LENGTH, 0, 0, 0, 0xaa, 0x55, ...u16le(tableEnd), totalCount];
    while (header.length < 23) header.push(0);
    const oldEventTails = existing.flatMap(m => {
        const needed = (m.data.length + 1) * 4;
        const cmd = m.cmd.map(v => typeof v === "string" ? parseInt(v, 16) : v);
        return cmd.slice(-needed);
    });
    const pointers = [];
